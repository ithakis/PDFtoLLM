...
\begin{tikzpicture}[
  ...
  box/.style={
    rectangle, rounded corners=3pt, draw=MSBlue, fill=MSBlue!8,
    minimum height=0.8cm, minimum width=1.4cm,
    text width=1.3cm, align=center, font=\tiny\bfseries,
    line width=0.6pt
  },
  arrow/.style={-{Stealth[length=4pt]}, thick, color=MSLightBlue},
...
\node[box, right=of md, fill=AccentAmber!20, draw=AccentAmber!80!black] (packet) {Figure\\Packets};

% Split paths
\node[box, above right=0.1cm and 0.4cm of packet, fill=AccentGreen!10, draw=AccentGreen!60!black] (deplot) {DePlot\\(Charts)};
\node[box, below right=0.1cm and 0.4cm of packet, fill=MSLightBlue!10, draw=MSLightBlue] (vlm) {VLM\\(Diagrams)};

% Merge
\node[box, right=2.8cm of packet] (complete) {Complete\\Markdown};
\node[box, right=of complete] (vdb) {Vector\\Database};

% Connections
\draw[arrow] (pdf) -- (marker);
\draw[arrow] (marker) -- (md);
\draw[arrow] (md) -- (packet);
\draw[arrow] (packet) |- (deplot);
\draw[arrow] (packet) |- (vlm);
\draw[arrow] (deplot) -| (complete);
\draw[arrow] (vlm) -| (complete);
\draw[arrow] (complete) -- (vdb);

% Context arrow
\draw[arrow, dashed, color=MSGrey] (md) to[bend right=25] node[below, font=\tiny] {Text Context} (complete);
...
\begin{column}{0.52\textwidth}
  \begin{block}{\small How It Works}
    \scriptsize
    \begin{itemize}
      \item \textbf{Figure Packets:} Context-aware extraction (Image + Caption + In-text mentions).
      \item \textbf{Chart Data:} \textbf{DePlot} converts plots to structured tables (not just pixels).
      \item \textbf{Diagrams:} Multimodal LLM provides structured JSON interpretation.
    \end{itemize}
  \end{block}
\end{column}
\begin{column}{0.44\textwidth}
  \begin{block}{\small The End-State Vision}
    ...
    \begin{itemize}
      ...
      \item \textbf{High-Fidelity Context:} LLM sees the data behind the charts.
...
\textcolor{MSLightBlue}{\textbf{DePlot}} \newline\scriptsize{(Google)} &
  \textbf{Chart $\to$ Table} \newline (data extraction) &
  \iconcheck\ Linearizes plots to text &
  GPU &
  Converts chart images into data tables for reasoning. \\
\midrule
...
\begin{exampleblock}{Strategy}
  \small \textbf{Marker} for text/math. \textbf{DePlot} for charts. \textbf{Nemotron} for deep reasoning and diagrams.
\end{exampleblock}
\end{frame}
...
\begin{column}{0.48\textwidth}
  \begin{block}{1. Ingestion Pipeline}
    \begin{itemize}
      \item \textbf{PDF Parsing:} \textbf{Marker (Surya)} for layout/math
      \item \textbf{Figure Packets:} Image + Caption + Context
      \item \textbf{Routing:} \textbf{DePlot} (Charts) \& \textbf{VLM} (Diagrams)
      \item \textbf{Quality Gates:} Consistency checks (e.g. caption match)
      \item \textbf{Result:} Full Markdown with structured data
    \end{itemize}
  \end{block}

  \begin{block}{2. Dual-Store Storage}
    \begin{itemize}
      \item \textbf{Vector DB:} Equation-aware chunks
      \item \textbf{Doc Store:} Faithful Markdown context
    \end{itemize}
  \end{block}
\end{column}
